...
async def send_group_message(bot: Bot, group_id: int, message: str | MessageSegment):
    try:
        ...
        nonebot.logger.warning(f'send to group {group_id} failed with retcode {getattr(e, "retcode")}')
    except Exception as e:
        nonebot.logger.warning(f'send to group {group_id} failed unknown error {e}')
...
async def send_private_message(bot: Bot, user_id: int, message: str | MessageSegment):
    try:
        ...
        nonebot.logger.warning(f'send to user {user_id} failed with retcode {getattr(e, "retcode")}')
    except Exception as e:
        nonebot.logger.warning(f'send to user {user_id} failed unknown error {e}')
